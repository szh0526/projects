 ...
 */
function clean() {
    return del([staticCfg.buildPath]);
}
...
function webpackBuild(callback) {
    var devConfig = getWebpackConfig();
...
function webpackDevServer(callback) {
    ...
    var server = new WebpackDevServer(devCompiler, {
        stats: { colors: true },
        contentBase: staticCfg.buildPath,
...
function startMockServer() {
    gulp.src(staticCfg.buildPath).pipe(
        webserver({
            livereload: true,
            directoryListing: {
                enable: true,
